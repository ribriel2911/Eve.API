...
{
    [TestClass]
    public class NextAsyncTest : PlayerBusinessTest<ChangeCriteria ,IChangeCriteria, Task<IMediaView>>
    {
        [TestInitialize]
        ...
        [TestMethod]
        public async Task Ok_NextOfFirst()
        {
            VlcMock.IsNowPlayingAsync.Setup(true);
            ...
            Assert.AreEqual(Samples.ElementAt(1).Name, second.Name);
            Assert.AreEqual("Radio", second.Type);
            Assert.IsTrue(second.Playing);
            Assert.IsNull(second.Duration);
            Assert.IsNull(second.Played);
            Assert.AreEqual(Samples.ElementAt(1).Frequency, second.Frequency);
            Assert.AreEqual(Samples.ElementAt(1).Wave.ToString("g"), second.Wave);
        ...
        public async Task Ok_RepeatAll()
        {
            ...
            while (i < Samples.Count())
            {
                ...
                Assert.AreEqual(Samples.ElementAt(i).Name, next.Name);
                Assert.AreEqual("Radio", next.Type);
                Assert.IsTrue(next.Playing);
                Assert.IsNull(next.Duration);
                Assert.IsNull(next.Played);
                Assert.AreEqual(Samples.ElementAt(i).Frequency, next.Frequency);
                Assert.AreEqual(Samples.ElementAt(i).Wave.ToString("g"), next.Wave);
        ...
        [TestMethod]
        public async Task Ok_NextOfFirst_Aleatory()
        {
            VlcMock.IsNowPlayingAsync.Setup(true);
